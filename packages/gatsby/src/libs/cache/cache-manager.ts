...
export class CacheManager {
  ...
  async get(
    ...
    if (!wrappedEntry) {
      ...
      const matchedKey = CacheManager.matchExistingCacheKey(cacheKey, keys);
  ...
  async set(entry: CacheEntry): Promise<void> {
    ...
    const wrappedEntry = CacheManager.wrapCacheEntry(entry);
  ...
  private static wrapCacheEntry(entry: CacheEntry): WrappedCacheEntry {
  ...
  private async getCacheKeys(): Promise<string[]> {
    const key = this.keyManifest ? await this.keyManifest.get() : undefined;
    return key ? key.keys : this.cache.allKeys();
  }
   ...
   */
  private static matchExistingCacheKey(keyToMatch: CacheKey, allKeys: Array<string>) {
