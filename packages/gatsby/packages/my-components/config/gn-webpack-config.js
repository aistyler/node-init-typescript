...
const upsert_rule = (config, rule) => {
  const idx = config.module.rules.findIndex((r) => r && r.test && r.test.toString() === rule.test.toString());
  if (idx >= 0) {
...
const customizeCss = (webpackConfig, { isDev = false, customCssModule = /\.module\.css$/, customScss = /\.scss$/, customScssModule = /\.module\.scss$/ }) => {
    //
    // support .module.css
    ...
    if (!cssRule) {
      cssRule = {
        test: /\.css$/,
        use: [
          isDev ? "style-loader" : require("mini-css-extract-plugin").loader,
          "css-loader",
        ],
      };
    ...
    if (customCssModule) {
      ...
      if (cssRule.exclude)
        cssRule.exclude.push(test);
      else
        cssRule.exclude = test;
...
module.exports = ({ actions, getConfig }) => {
  ...
  console.log(">>>", JSON.stringify(config.module.rules, function(k,v) { return k === "test" ? v.toString() : v; }, 2));
};
